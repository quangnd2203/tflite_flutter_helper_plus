...
import 'package:tflite_flutter_helper_plus/src/common/support_preconditions.dart';
import 'package:tflite_flutter_helper_plus/src/common/tensor_operator.dart';
...
class NormalizeOp implements TensorOperator {
  ...
  NormalizeOp(double mean, double stddev) {
    ...
    isIdentityOp = meansIsZeroAndDevsIs1;
    this.mean = [mean];
    this.stddev = [stddev];
    numChannels = 1;
  }
  ...
  NormalizeOp.multipleChannels(List<double> mean, List<double> stddev) {
    ...
    SupportPreconditions.checkArgument(mean.isNotEmpty,
    ...
    isIdentityOp = allMeansAreZeroAndAllDevsAre1;
  }
  ...
  TensorBuffer apply(TensorBuffer input) {
    ...
    SupportPreconditions.checkArgument(
        numChannels == 1 ||
            (shape.isNotEmpty && shape[shape.length - 1] == numChannels),
        errorMessage:
